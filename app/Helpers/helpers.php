...
function siteTitle()
{
    return (ucfirst(getGen()->sitename));
}
...
function posts($command = false)
{
    if (!$command) {
        return Cache::remember("posts", 60 * 5, function () {
            return CryptoNews::query()->select('data')->take(50)->get();
        });
    }
    return Cache::remember("posts", 60 * 5, function () {
        $client = new Client();
...
function categories()
{
    try {
        return Cache::remember("categories", 60 * 5, function () {
            $endpoint = "https://jupitarfinance.com/api/thecollectionv1/categories";
