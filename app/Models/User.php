...
use App\Utility\ManipulateDate;
...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    public function p2pSeller()
    {
        return $this->hasOne(P2PSeller::class);
    }
    ...
    public function unilevel_downlines()
    {
        return Cache::remember($this->cacheKey($this) . ':unilevel_downlines', now()->addHours(4), function () {
            $downlines = $this->where('ref_by', $this->id)
                ->with(['mlm', 'downlines'])
                ->select('id', 'firstname', 'lastname', 'username', 'profile_photo_path', 'created_at')
                ->get();

            // Prepare the result array
            $preparedDownlines = [];

            // Loop through all downlines and call prepareTreeData for each of them
            foreach ($downlines as $downline) {
                $preparedDownlines[] = $this->prepareTreeData($downline);
            }

            return $preparedDownlines;
        });
    }


    public function prepareTreeData($downline)
    {
        if ($downline === null) {
            return null;
        }
        ...
        $treeData = [
            ...
            'joinedAt' => $downline->created_at ?: '',
            'isActive' => $downline->membership === 1,
            'profilePhotoPath' => $downline->profile_photo_path ?: 'default.png',
            'rank' => $downline->mlm->rank,
            'children' => [],
        ];

        if ($downline->downlines && count($downline->downlines) > 0) {
            foreach ($downline->downlines as $downline) {
                $treeData['children'][] = $this->prepareTreeData($downline);
            }
        }
    ...
    public function allusers($get = 30)
    {
        ...
        $data['last_week'] = Cache::remember($this->cacheKey($this) . ':all_users_last_week', now()->addHours(4), function () {
            return self::whereBetween('created_at', [ManipulateDate::startOfWeek(), ManipulateDate::endOfWeek()])->count();
        });
    ...
    public function verified($get = 30)
    {
        ...
        $data['last_week'] = Cache::remember($this->cacheKey($this) . ':verified_last_week', now()->addHours(4), function () {
            return self::where('email_verified_at', '!=', null)->whereBetween('created_at', [ManipulateDate::startOfWeek(), ManipulateDate::endOfWeek()])->count();
        });
    ...
    {
        return $this->hasOne('App\Models\PlanLog', 'user_id');
    }

    public function getPlanAttribute()
    {
        return $this->userplan['pricing_plan_id'];
    }
    public function ccy()
    {
