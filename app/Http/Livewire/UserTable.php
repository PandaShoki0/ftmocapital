...
class UserTable extends DataTableComponent
{
            ...
            ButtonGroupColumn::make('Actions')
                ...
                ->buttons([
                    ...
                    LinkColumn::make('Contact')
                        ...
                        LinkColumn::make('Manage Trade')
                        ->title(fn ($row) => 'Manage Trade')
                        ->location(fn ($row) => route('admin.users.manage', $row->id))
                        ->attributes(function ($row) {
                            return [
                                'class' => 'btn btn-outline-warning btn-sm',
                            ];
                        }),
                        LinkColumn::make('Manage User')
                        ->title(fn ($row) => 'Manage User')
                        ->location(fn ($row) => route('admin.impersonate.in', $row->id))
                        ->attributes(function ($row) {
                            return [
                                'class' => 'btn btn-outline-warning btn-sm',
                                'target' => '_blank'
    ...
    public function delete()
    {
        ...
        foreach ($this->getSelected() as $id) {
            ...
            $tradePredictions = TradePrediction::where('user_id', $id)->get();
            foreach ($tradePredictions as $tradePrediction) {
                $tradePrediction->delete();
            }
