...
class ExchangeController extends Controller
{
    ...
    {
        $model = $this->provider ? ThirdpartyOrders::class : ExchangeLogs::class;
        $thirdparty_model = ThirdpartyOrders::where('user_id', $userId)
        ->where('provider', $this->provider ?? 'local')
        ->where('symbol', $symbol . '/' . $currency)
        ->whereIn('status', $status)
        ->latest()
        ->get();
        $local_model = ExchangeLogs::where('user_id', $userId)
        ...
        $result = $thirdparty_model->merge($local_model);
        // dd($thirdparty_model, $local_model);
        return $result;
