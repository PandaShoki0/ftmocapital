...
class InstallController extends Controller
{
    ...
    {
        $product = Extension::where('product_id', $product_id)->first();
        $page_title = $product->name . ' Installer';
        return view('admin.extension.install', compact('page_title', 'product', 'product_id'));
    }
    ...
    public function activater($product_id)
    {
        ...
        return view('admin.extension.activate', compact('page_title', 'product'));
    }
    ...
    public function activate_licenser(Request $request)
    {
        ...
        if (!empty($create_lic)) {
            if ($response['status']) {
                ...
                $ext = Extension::where('product_id', $request->product_id)->first();
                $ext->activated = '1';
                $ext->save();
                $notify[] = ['success', 'Extension activated successfully'];
            ...
            if (is_array($db_for_import)) {
                if (!empty($db_for_import["db_host"]) && !empty($db_for_import["db_user"]) && !empty($db_for_import["db_name"])) {
                    ...
                    if (mysqli_connect_errno()) {
                        ...
                        $ext = Extension::where('product_id', $product_id)->first();
                        $ext->installed = '1';
                        $version = str_replace("_", ".", $version);
