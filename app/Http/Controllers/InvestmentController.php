        ...
        try {
            ...
            foreach ($logs as $log) {
                ...
                if ($log->last_profit_calculation == null && $log->end_date <= $now) {
                    ...
                    if ($log->plan && $log->plan->interest_rate) {
                        $profit = $log->amount * ($log->plan->interest_rate / 100) * $total_days;
                    } else {
                        // Handle the case when plan or interest_rate is null
                        $profit = 0; // or any other default value or behavior
                    }
                    ...
                    if ($now >= $next_calculation_date && $log->plan) {
