    ...
    <div class="py-3 px-3 lg:px-5 lg:pl-3">
        <div class="flex justify-between items-center">
            <div class="flex justify-start items-center">
                ...
                @if (!$agent->isMobile())
                    <img id="logo" src="{{ getImage(imagePath()['logoIcon']['path'] . '/bt/logo.png') }}"
                        class="mr-3 h-8" />
                @endif
            </div>
            <div class="flex items-center">
                <livewire:components.panels.admin-notifications />

                <a href="{{ route('app.home') }}" class="btn btn-outline-secondary mx-3">
                    @if ($agent->isMobile())
                        <i class="bi bi-person" style="margin-right:0px !important"></i>
                    @else
                        {{ __('User Panel') }}
                    @endif
                </a>
