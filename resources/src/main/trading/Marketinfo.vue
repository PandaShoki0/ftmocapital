  ...
  export default {
    ...
    methods: {
      ...
      async updateTicker(tick) {
        ...
        this.last_price = this.priceFormatter(tick["last"]) || this.last_price;
        ...
        if (this.provide != "coinbasepro") {
          ...
          this.day_change = tick["percentage"] ? Number(tick["percentage"]).toFixed(2) : this.day_change;

          this.day_volume_currency = this.priceFormatter(
            tick["quoteVolume"],
            2
          ) || this.day_volume_currency;
        }
        
        this.day_high = this.priceFormatter(tick["high"]) || this.day_high;
        this.day_low = this.priceFormatter(tick["low"]) || this.day_low;
        this.day_volume_pair = this.priceFormatter(tick["baseVolume"], 2) || this.day_volume_pair;
      },
      async loopTicker() {
