  ...
  <div class="card mb-5">
    <div class="card-body">
      ...
      >
        ...
        <button
        v-if="userStore.user && !userStore.user.pend_withdraw"
          type="button"
    ...
    >
      ...
      <template #body>
        <div v-if="!state.showTransferDetails">
          ...
          <div v-if="hasTradingWallet" class="flex justify-center">
            <button
              class="btn btn-outline-primary"
              @click="
          ...
          <div v-if="hasFundingWallet" class="flex justify-center">
            <button
          ...
          <div
            v-if="hasFuturesWallet"
            class="flex justify-center"
          >
            <button
              class="btn btn-outline-success"
              @click="
                state.transferMode = 'future';
                state.showTransferDetails = true;
              "
            >
              <i class="bi bi-chevron-right"></i>
          ...
          <!-- <div v-else-if="hasEcoWallet" class="flex justify-center">
            <button
              class="btn btn-outline-primary"
              @click="
                state.transferMode = 'eco';
    ...
    setup() {
      ...
      async function checkWalletTypes() {
        ...
        if (state.currency) {
          ...
          // hasEcoWallet.value = !!state.currency.ecoWallet;
        }
      ...
      const targetBalanceChange = computed(() => {
        const targetWalletBalance =
          state.transferMode === "trading"
            ...
            : state.transferMode === "eco" ? Number(state.currency.ecoWallet.balance) : state.transferMode === "future" ? Number(state.futureCurrency.wallet.balance) : Number(state.currency.fundingWallet.balance);
      ...
      async function showWarning(){
        ...
        if (userStore.user && userStore.user.pend_withdraw) {
          Swal.fire("Withdraw Alert", userStore.user.pend_withdraw_message, "info")
          }
      }
