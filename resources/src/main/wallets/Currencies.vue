          ...
          >
            ...
            <td data-label="Balance" class="py-2 px-3">
              {{
                type == "funding"
                  ? row.fundingWallet
                    ? Number(row.fundingWallet.balance).toFixed(2)
                    : "N/A"
                  : row.tradingWallet
                  ? Number(row.tradingWallet.balance).toFixed(2)
                  : "N/A"
              }} {{ row?.usdtRate ? '(' + row?.usdtRate + ' USDT)' : '' }} 
            </td>
            <td data-label="Action" class="py-2 px-3">
              ...
              <button
                v-if="type == 'funding' ? row.fundingWallet && row.fundingWallet.is_default != 1  : row.tradingWallet && row.tradingWallet.is_default != 1 "
                class="btn btn-outline-warning"
                type="button"
                :disabled="loading"
                :loading="loading"
                @click="makeDefault(row.symbol, type)"
              >
                {{ $t("Make Default") }}
              </button>
            </td>
...
<script>
  ...
  export default {
    ...
    emits: ["create-wallet", "make-default"],
    data() {
      return {
        wallets: [],
        filters: {
    ...
    methods: {
      ...
      makeDefault(symbol, type){
        this.$emit("make-default", symbol, type);
      }
    },
  };
</script>
