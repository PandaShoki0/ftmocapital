      ...
      <div class="card relative">
        <div class="card-body flex justify-between">
          <div>
            ...
            <router-link :to="'#'" type="button" @click.prevent="handleBotClick" class="btn btn-primary my-3">{{
              $t("New Bot") }}</router-link>
          </div>
          <img src="/assets/images/illustrations/bot.svg" class="w-48" alt="Medal Pic" />
        </div>
      </div>
      <div class="grid gap-5 xs:grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
        <div class="card text-center">
          <div class="p-5">
            ...
            <h2 v-if="botStore.bot_contracts_count_running != null" :key="botStore.bot_contracts_count_running"
              class="font-medium">
        ...
        <div class="card text-center">
          <div class="p-5">
            <div class="my-3">
              <span class="icon-bg rounded-full bg-gray-100 p-2 shadow dark:border dark:bg-gray-700 dark:shadow-lg">
                <i class="bi bi-check-lg text-xl font-medium"></i>
              </span>
            </div>
            <h2 v-if="botStore.bot_contracts_count_completed != null" :key="botStore.bot_contracts_count_completed"
              class="font-medium">
        ...
        <div class="card text-center">
          <div class="p-5">
            ...
            " :key="botStore.bot_contracts_count_amount" class="font-medium">
              {{
                botStore.bot_contracts_count_amount * userStore.currency.rate
              }}
        ...
        <div class="card text-center">
          <div class="p-5">
            <div class="my-3">
              <span class="icon-bg rounded-full bg-gray-100 p-2 shadow dark:border dark:bg-gray-700 dark:shadow-lg">
                <i class="bi bi-arrows-expand text-xl font-medium"></i>
              </span>
            </div>
            <span>
              ...
              <h2 v-else-if="botStore.profit < 0" class="text-danger font-medium">
                {{
                  botStore.profit *
                  (userStore.currency ? userStore.currency.rate : 1)
                }}
    ...
    <div class="mt-10">
      <div class="mb-4 items-center justify-between xs:block xs:space-y-5 sm:flex sm:space-y-0">
        ...
        <div class="grid grid-cols-2 gap-5">
          ...
          <Filter class="if-sm">
            <input v-model="filters.pair.value" type="text" class="filter-input" placeholder="Pair" />
          </Filter>
        </div>
      </div>
      <div class="card relative overflow-x-auto">
        <VTable :key="botStore.logs.length" v-model:current-page="currentPage"
          class="w-full text-left text-sm text-gray-500 dark:text-gray-400" :data="botStore.logs" :filters="filters"
          :page-size="10" :hide-sort-icons="true" @totalPagesChanged="totalPages = $event">
          <template #head>
            <tr class="bg-gray-100 text-xs uppercase text-gray-700 dark:bg-gray-700 dark:text-gray-400">
              ...
              <VTh sort-key="duration" scope="col" class="hidden py-3 px-6 md:flex">
                <Col text="Duration" />
              </VTh>
          ...
          <template #body="{ rows }">
            <template v-if="botStore.logs.length > 0">
              <tr v-for="row in rows" :key="row.id" class="border-b bg-white dark:border-gray-700 dark:bg-gray-800">
                ...
                <td class="hidden py-3 px-6 md:block" data-label="Duration">
                  <div>
                    {{ $t("Start") }}:
                ...
                <td class="py-3 px-6" data-label="Status">
                  <template v-if="row.status != 1">
                    <span class="badge bg-warning">{{ $t("Running") }}</span>
                  </template>
                  <template v-else>
                    <span v-if="row.result == 1" class="badge bg-success">+
                      <toMoney :num="row.profit" decimals="4" />
                    </span>
                    <span v-else-if="row.result == 2" class="badge bg-danger">-
                      <toMoney :num="row.profit" decimals="4" />
                    </span>
                    <span v-else-if="row.result == 3" class="badge bg-secondary">
                      <toMoney :num="row.profit" decimals="4" />
                    </span>
                  </template>
                </td>
                <td class="py-3 px-6" data-label="Status">
                  <span v-if="row.status != 1" class="badge bg-warning">{{
                    $t("Running")
                  }}</span>
                ...
                <td class="py-3 px-6" data-label="View">
                  <router-link v-if="row.status != 1" :to="'/bot/' + row.symbol + '/' + row.pair"
      ...
      <VTPagination v-model:currentPage="currentPage" aria-label="Table navigation" :total-pages="totalPages"
        :boundary-links="true" :max-page-links="3" class="float-right flex items-center justify-between pt-4">
...
export default {
  ...
  setup() {
    ...
    const newBotUrl = '/bot/BTC/USDT';
    const handleBotClick = async (e) => {
      e.preventDefault();
      if (hasLimitExceeded() || hasPend() || hasLicensePrompt()) {
        await showAllAlertsSequentially();
      } else {
        router.push(newBotUrl);
      }
    }

    return { userStore, botStore, handleBotClick, newBotUrl };
  },
  data() {
    return {
      gnl: gnl,
