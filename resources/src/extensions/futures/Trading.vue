...
<script>
  ...
  export default {
    ...
    setup() {
      ...
      if (marketStore.futuresExchange === null) {
        if (provider !== "bitget") {
          marketStore.futuresExchange = marketStore.exchange;
        } else {
          const futuresProvider =
            provider !== "kucoin" ? provider : "kucoinfutures";
          marketStore.futuresExchange = new ccxt.pro[futuresProvider](config);
        }
      }

      const initializeMarket = async () => {
        if (marketStore.futures.length === 0) {
          await marketStore.fetch_futures();
        }
        marketStore.future =
          marketStore.futures[pair][currency + "/" + pair + ":" + pair];
      ...
      async function checkKyc() {
        if (
          ...
          Number(plat.kyc.futures_restriction) === 1
        ) {
          if (!userStore.user) {
      ...
      fetchData() {
        axios
          ...
          .then((response) => {
            ...
            (this.provide = response.provide), (this.fee = response.fee);
          });
      },
    },
  };
